...
def get_quanta_emmission_rate(activity, expiratory_activity, mask_tf, var = var):
    ...
    if mask_tf:
       ...
       #Source: https://doi.org/10.1016/j.ajic.2007.07.008
        summation = sum([(1 - var['mask_efficacy']['.8um']) * Dc['.8um'] * (Dv['.8um'] * CUBIC_μM_TO_CUBIC_CM),
                     (1 - var['mask_efficacy']['1.8um']) * Dc['1.8um'] * (Dv['1.8um'] * CUBIC_μM_TO_CUBIC_CM),
                     (1 - var['mask_efficacy']['3.5um']) * Dc['3.5um'] * (Dv['3.5um'] * CUBIC_μM_TO_CUBIC_CM),
                     (1 - var['mask_efficacy']['5.5um']) * Dc['5.5um'] * (Dv['5.5um'] * CUBIC_μM_TO_CUBIC_CM)])
    else:
        summation = sum([Dc['.8um'] * (Dv['.8um'] * CUBIC_μM_TO_CUBIC_CM),
                     Dc['1.8um'] * (Dv['1.8um'] * CUBIC_μM_TO_CUBIC_CM),
                     Dc['3.5um'] * (Dv['3.5um'] * CUBIC_μM_TO_CUBIC_CM),
                     Dc['5.5um'] * (Dv['5.5um'] * CUBIC_μM_TO_CUBIC_CM)])
